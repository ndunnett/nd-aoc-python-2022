...
def puzzle():
    ...
    modulus = reduce(lambda x, y: x * y, [monkey.relief * monkey.test for monkey in monkeys])

    for _ in range(20):
        for monkey in monkeys:
            for item in monkey.items:
                worry = monkey.operation(item) // monkey.relief % modulus
                throw_to = monkey.true if worry % monkey.test == 0 else monkey.false
